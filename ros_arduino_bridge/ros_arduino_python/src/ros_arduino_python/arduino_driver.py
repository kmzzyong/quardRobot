...
class Arduino:
    ...
    def update_pid(self, Aleft_Kp, Aleft_Kd, Aleft_Ki, Aleft_Ko,Aright_Kp, Aright_Kd, Aright_Ki, Aright_Ko,Bleft_Kp, Bleft_Kd, Bleft_Ki, Bleft_Ko,Bright_Kp, Bright_Kd, Bright_Ki, Bright_Ko):
        ''' Set the PID parameters on the Arduino
        '''
        print "Updating PID parameters"
        cmd = 'u ' + str(Aleft_Kp) + ':' + str(Aleft_Kd) + ':' + str(Aleft_Ki) + ':' + str(Aleft_Ko)+ ':' + str(Aright_Kp) + ':' + str(Aright_Kd) + ':' + str(Aright_Ki) + ':' + str(Aright_Ko)+ ':' + str(Bleft_Kp) + ':' + str(Bleft_Kd) + ':' + str(Bleft_Ki) + ':' + str(Bleft_Ko)+ ':' + str(Bright_Kp) + ':' + str(Bright_Kd) + ':' + str(Bright_Ki) + ':' + str(Bright_Ko)
    ...
    def drive(self, Aleft,Aright,Bleft,Bright):
        ''' Speeds are given in encoder ticks per PID interval
        '''
        if self.motors_reversed:
            Aleft, Aright,Bleft, Bright = -Aleft, -Aright,-Bleft, -Bright
        return self.execute_ack('m %d %d %d %d' %(Aleft,Aright,Bleft,Bright))

    def drive_m_per_s(self, Aleft,Aright,Bleft,Bright):
        ''' Set the motor speeds in meters per second.
        '''
        Aleft_revs_per_second = float(Aleft) / (self.wheel_diameter * PI)
        Aright_revs_per_second = float(Aright) / (self.wheel_diameter * PI)
        Bleft_revs_per_second = float(Bleft) / (self.wheel_diameter * PI)
        Bright_revs_per_second = float(Bright) / (self.wheel_diameter * PI)

        Aleft_ticks_per_loop = int(Aleft_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        Aright_ticks_per_loop  = int(Aright_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        Bleft_ticks_per_loop = int(Bleft_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        Bright_ticks_per_loop  = int(Bright_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)

        self.drive( Aleft_ticks_per_loop,Aright_ticks_per_loop ,Bleft_ticks_per_loop ,Bright_ticks_per_loop , )
